...
- **`Models/SaveData.cs`** — **classe**, pas struct : elle porte deux `List<>`, et une copie de struct partagerait les références en donnant l'illusion d'une copie indépendante. `CurrentVersion = 5`, `EnsureValid()` (répare une désérialisation), `Migrate()` (cas chaînés par `goto case`).
